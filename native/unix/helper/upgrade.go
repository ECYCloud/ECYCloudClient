...
// 发布包里需要落地的文件及其安装后的名字，与 scripts/fetch-kernel.sh 的 files 一致
...
// 只有内核本身需要执行位
var payloadModes = map[string]os.FileMode{"sing-box": 0o755}
...
func (s *server) upgradeKernel(version string) (string, error) {
	...
	installed := kernelVersionOf(staged["sing-box"])
	if installed == "" {
...
func resolveKernelAsset(version string) (name string, url string, sha256Hex string, err error) {
	...
	wanted := fmt.Sprintf("sing-box-%s-%s-%s.tar.gz", version, runtime.GOOS, runtime.GOARCH)
...
func extractKernel(archive, dir string) (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("发布包无法读取: %w", err)
	}
	defer file.Close()
	...
	for {
		...
		if !ok || entry.Typeflag != tar.TypeReg || staged[target] != "" {
			continue
		}

		path := filepath.Join(dir, target)
		if err := extractOne(reader, path, payloadModes[target]); err != nil {
			return nil, err
		}
		staged[target] = path
	}
	...
	if staged["sing-box"] == "" {
		return nil, fmt.Errorf("发布包中没有 sing-box")
	}
	return staged, nil
}
