...
# 用法: build-linux.sh [--arch x64|arm64] [--version 1.0.1] [--panel-url https://面板域名] [--package] [--format deb,rpm,tar]
set -euo pipefail
...
# 进 /opt/ecycloud，helper 按自身目录定位 sing-box，GUI 路径也固定在这里（verifyGUICaller 只认它）
