...
install -m 0755 "$here/purge-data.sh" "$target/purge-data.sh"
if [ -f "$here/opt/ecycloud/link-libmpv.sh" ]; then
    install -m 0755 "$here/opt/ecycloud/link-libmpv.sh" "$target/link-libmpv.sh"
fi
"$target/link-libmpv.sh" || true
...
if [ ! -e /opt/ecycloud/lib/libmpv.so.1 ] && ! ldconfig -p 2>/dev/null | grep -q 'libmpv\.so'; then
    echo "缺少 libmpv，客户端无法启动，请用发行版包管理器安装 libmpv2 或 libmpv1" >&2
fi

echo "安装完成，可从桌面菜单启动 ECY Cloud。卸载：sudo $target/uninstall.sh（会询问是否删除数据）"
