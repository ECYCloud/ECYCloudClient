...
# media_kit_video 链 libmpv.so.1；仅有 so.2 时 %post 在 /opt/ecycloud/lib 补兼容名
Recommends:     (libmpv.so.2()(64bit) or libmpv.so.1()(64bit))
...
%post
if [ -x /opt/ecycloud/link-libmpv.sh ]; then
    /opt/ecycloud/link-libmpv.sh
fi
...
# 卸包默认保留数据；要清数据须先 sudo /opt/ecycloud/purge-data.sh
