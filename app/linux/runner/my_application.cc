...
static constexpr const char kWindowTitle[] =
    "ECY Cloud " ECYCLOUD_DISPLAY_VERSION;
...
static void my_application_activate(GApplication* application) {
  ...
  // 缩到托盘后再次启动会走到这里，此时应还原已有窗口而不是再开一个
  GList* existing = gtk_application_get_windows(GTK_APPLICATION(application));
  if (existing != nullptr) {
    gtk_widget_show(GTK_WIDGET(existing->data));
...
static void my_application_dispose(GObject* object) {
  ...
  g_clear_pointer(&self->platform_channel, platform_channel_free);
