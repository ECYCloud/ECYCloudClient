...
class ServiceKernelController implements KernelController {
  ServiceKernelController(this._pipe);

  factory ServiceKernelController.production() =>
      ServiceKernelController(ServicePipe.production());
  ...
  Future<String> kernelVersion() async {
    try {
      final Map<String, dynamic> pong = await _pipe.request('ping');
  ...
  Future<String> upgrade(String version) async {
    ...
    try {
      final Map<String, dynamic> result = await _pipe.request(
        'kernel.upgrade',
  ...
  Future<List<String>> preflight() async {
    ...
    try {
      final Map<String, dynamic> result = await _pipe.request('wintun.ensure');
      if (result['ready'] != true) {
        final String reason = result['reason'] as String? ?? '未知原因';
        problems.add('TUN 模式不可用：$reason');
      }
    } on PipeException catch (e) {
      problems.add('wintun 检查失败：$e');
    }
  ...
  Future<void> start({
    ...
    try {
      await _pipe.request('kernel.start', <String, dynamic>{
        'config': configJson,
      });
    } on PipeException catch (e) {
  ...
  Future<void> stop() async {
    ...
    try {
      await _pipe.request('kernel.stop');
    } on PipeException catch (e) {
  ...
  Future<void> _poll() async {
    ...
    try {
      result = await _pipe.request('kernel.status', <String, dynamic>{
