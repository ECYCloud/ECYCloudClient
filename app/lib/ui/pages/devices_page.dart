import 'dart:async';

import 'package:flutter/material.dart';

import '../../data/api/api_exception.dart';
import '../../data/api/panel_api_client.dart';
import '../../data/models/account.dart';
import '../app_scope.dart';
import '../auto_refresh_mixin.dart';
import '../theme.dart';
import '../widgets/list_toolbar.dart';
import '../widgets/page_header.dart';
import '../widgets/refresh_button.dart';
import '../widgets/simple_data_table.dart';
import '../widgets/tag_chip.dart';
import '../../l10n/l10n.dart';

class DevicesPage extends StatefulWidget {
  const DevicesPage({super.key});

  @override
  State<DevicesPage> createState() => _DevicesPageState();
}

class _DevicesPageState extends State<DevicesPage>
    with AutoRefreshMixin<DevicesPage> {
  final List<DeviceListItem> _items = <DeviceListItem>[];
  int _page = 1;
  int _lastPage = 1;
  int _total = 0;
  int _logKeepDays = 1;
  String _search = '';
  String? _error;
  bool _busy = false;
  bool _started = false;
  Timer? _searchDebounce;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (!_started) {
      _started = true;
      startAutoRefresh(() => _load(silent: true));
      unawaited(_load());
    }
  }

  @override
  void dispose() {
    _searchDebounce?.cancel();
    super.dispose();
  }

  Future<void> _load({bool silent = false}) async {
    final PanelApiClient? api = AppScope.of(context).auth.api;
    if (api == null || (silent && _busy)) {
      return;
    }
    if (!silent) {
      setState(() {
        _busy = true;
        _error = null;
      });
    }
    await loadLatest(
      () => api.fetchDevices(
        page: _page,
        length: ListToolbar.pageSize,
        search: _search,
      ),
      silent: silent,
      onData: (DeviceListPage result) {
        setState(() {
          _items
            ..clear()
            ..addAll(result.items);
          _page = result.currentPage;
          _lastPage = result.lastPage;
          _total = result.total;
          _logKeepDays = result.logKeepDays > 0 ? result.logKeepDays : 1;
          if (!silent) {
            _busy = false;
          }
          _error = null;
        });
      },
      onError: (Object e) {
        setState(() {
          _error = e is ApiException
              ? e.message
              : L10n.t('加载失败：{0}', <Object>[e]);
          _busy = false;
        });
      },
    );
  }

  Future<void> _kick(DeviceListItem item) async {
    final PanelApiClient? api = AppScope.of(context).auth.api;
    if (api == null) {
      return;
    }
    final String label = item.device.isEmpty ? item.ip : item.device;
    final bool? confirmed = await showDialog<bool>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: Text(L10n.t('确认移除该设备？')),
        content: Text(L10n.t('将通知 {0} 上的客户端下线，最长 60 秒后生效。', <Object>[label])),
        actions: <Widget>[
          FilledButton(
            onPressed: () => Navigator.of(context).pop(true),
            child: Text(L10n.t('确定')),
          ),
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: Text(L10n.t('取消')),
          ),
        ],
      ),
    );
    if (confirmed != true || !mounted) {
      return;
    }
    String message;
    try {
      message = await api.kickDevice(item.deviceId);
    } on ApiException catch (e) {
      message = e.message;
    }
    if (!mounted) {
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(message), behavior: SnackBarBehavior.floating),
    );
    unawaited(_load());
  }

  void _onSearch(String value) {
    _searchDebounce?.cancel();
    _searchDebounce = Timer(const Duration(milliseconds: 300), () {
      _search = value.trim();
      _page = 1;
      unawaited(_load());
    });
  }

  Widget _deviceCell(ThemeData theme, DeviceListItem item, String selfId) {
    if (item.device.isEmpty) {
      return TableText(L10n.t('第三方客户端'));
    }
    final bool current = item.deviceId.isNotEmpty && item.deviceId == selfId;
    return Wrap(
      crossAxisAlignment: WrapCrossAlignment.center,
      spacing: 6,
      runSpacing: 4,
      children: <Widget>[
        TableText(item.device),
        if (current)
          TagChip(
            label: L10n.t('当前设备'),
            color: theme.colorScheme.primary,
          ),
      ],
    );
  }

  @override
  Widget build(BuildContext context) {
    final ThemeData theme = Theme.of(context);
    final String selfId = AppScope.of(context).auth.api?.device.deviceId ?? '';
    return Scaffold(
      body: Column(
        children: <Widget>[
          SafeArea(
            bottom: false,
            child: PageHeader(
              title: L10n.t('最近 {0} 天的在线客户端', <Object>[_logKeepDays]),
              showBackButton: true,
              showUserAvatar: true,
              actions: <Widget>[
                RefreshButton(tooltip: L10n.t('刷新'), onRefresh: _load),
              ],
            ),
          ),
          Expanded(
            child: RefreshIndicator(
              onRefresh: _load,
              child: ListView(
                physics: const AlwaysScrollableScrollPhysics(),
                padding: AppTheme.pageScrollPadding.copyWith(left: 0, top: 0),
                children: <Widget>[
                  Padding(
                    padding: const EdgeInsets.fromLTRB(14, 8, 14, 0),
                    child: Align(
                      alignment: Alignment.centerLeft,
                      child: Text(
                        L10n.t('请确认均为本人 IP，如有异常请及时修改密码。'),
                        style: theme.textTheme.bodySmall?.copyWith(
                          color: theme.colorScheme.onSurfaceVariant,
                          fontSize: AppTheme.pageHintSize,
                        ),
                      ),
                    ),
                  ),
                  ListToolbar(
                    currentPage: _page,
                    lastPage: _lastPage,
                    total: _total,
                    searchHint: L10n.t('IP / 归属地 / 设备 / 版本'),
                    onSearchChanged: _onSearch,
                    onPageChanged: (int page) {
                      _page = page;
                      unawaited(_load());
                    },
                  ),
                  if (_busy && _items.isEmpty)
                    const Padding(
                      padding: EdgeInsets.only(top: 80),
                      child: Center(child: CircularProgressIndicator()),
                    )
                  else if (_error != null && _items.isEmpty)
                    Padding(
                      padding: const EdgeInsets.only(top: 80),
                      child: Center(child: Text(_error!)),
                    )
                  else
                    Padding(
                      padding: EdgeInsets.only(
                        left: AppTheme.pageScrollPadding.left,
                      ),
                      child: SimpleDataTable(
                        minWidth: 1080,
                        columns: <String>[
                          'IP',
                          L10n.t('归属地'),
                          L10n.t('最后使用时间'),
                          L10n.t('在线设备'),
                          L10n.t('客户端版本'),
                          L10n.t('操作'),
                        ],
                        emptyText: L10n.t('最近 {0} 天没有在线客户端记录', <Object>[
                          _logKeepDays,
                        ]),
                        rows: <List<Widget>>[
                          for (final DeviceListItem item in _items)
                            <Widget>[
                              TableText(item.ip),
                              TableText(item.location),
                              TableText(item.datetime, muted: true),
                              _deviceCell(theme, item, selfId),
                              TableText(item.appVersion, muted: true),
                              item.online &&
                                      item.device.isNotEmpty &&
                                      item.deviceId.isNotEmpty
                                  ? FilledButton(
                                      onPressed: () => unawaited(_kick(item)),
                                      style: FilledButton.styleFrom(
                                        backgroundColor: AppTheme.danger,
                                      ),
                                      child: Text(L10n.t('移除')),
                                    )
                                  : TableText(
                                      item.device.isEmpty ? L10n.t('不支持') : '',
                                      muted: true,
                                    ),
                            ],
                        ],
                      ),
                    ),
                ],
              ),
            ),
          ),
        ],
      ),
    );
  }
}
