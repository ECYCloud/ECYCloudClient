...
class _HomePageState extends State<HomePage> {
  ...
  void didChangeDependencies() {
    ...
    if (!_requestedConnectFocus && AppScope.of(context).platform.isTelevision) {
      ...
      builder: (BuildContext context, _) {
        ...
        return OverlayScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              PageHeader(title: L10n.t('首页'), showUserAvatar: false),
              Padding(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    if (connection.routeMode == 'direct' ||
                        connection.groupsForMode.isNotEmpty) ...<Widget>[
                      const SizedBox(height: 10),
                    ...
                    const SizedBox(height: 10),
                    _TrafficRankCard(connection: connection),
                    const SizedBox(height: 10),
                      ...
                      InkWell(
                        onTap:
                            phase == ConnectionPhase.failed &&
                                (connection.formatError ?? '').trim().isNotEmpty
                            ? () {
                                unawaited(
                                  showProfileFormatDialog(
                                    context,
      ...
      child: SegmentedButton<String>(
        ...
        onSelectionChanged: enabled
            ? (Set<String> selection) =>
                  connection.setRouteMode(selection.first)
            : null,
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _TrafficBarRow(
            ...
            tagColor: remainBytes < 0 ? AppTheme.danger : AppTheme.success,
          ),
          const Padding(
          ...
          Row(
            children: <Widget>[
              ...
              Text(
                Format.bytes(user.transferEnable).replaceAll(' ', ''),
                style: _trafficAmountStyle(theme, theme.colorScheme.primary),
              ),
            ],
          ),
...
TextStyle _trafficAmountStyle(ThemeData theme, Color color) {
      ...
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            Text(value, style: _trafficAmountStyle(theme, tagColor)),
          ],
        ),
...
class _TrafficCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<TrafficSample> samples = connection.trafficHistory;
    final List<int> history = <int>[
      for (final TrafficSample sample in samples)
        upload ? sample.up : sample.down,
    ];
    final List<DateTime> times = <DateTime>[
      for (final TrafficSample sample in samples)
        sample.at ?? DateTime.fromMillisecondsSinceEpoch(0),
    ];
    ...
    return SectionCard(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          const SizedBox(height: 8),
          Sparkline(values: history, times: times, color: color),
        ],
      ),
    );
  }
...
class _TrafficRankCard extends StatefulWidget {
  const _TrafficRankCard({required this.connection});

  final ConnectionController connection;

  @override
  State<_TrafficRankCard> createState() => _TrafficRankCardState();
}

class _TrafficRankCardState extends State<_TrafficRankCard> {
  TrafficRankDay _day = TrafficRankDay.today;

  @override
  Widget build(BuildContext context) {
    final ThemeData theme = Theme.of(context);
    final List<TrafficRankRow> groups = widget.connection.trafficRankGroups(
      _day,
    );
    final List<TrafficRankRow> hosts = widget.connection.trafficRankHosts(_day);
      ...
      child: Column(
        ...
        children: <Widget>[
          Align(
            ...
            child: SegmentedButton<TrafficRankDay>(
              segments: <ButtonSegment<TrafficRankDay>>[
                ButtonSegment<TrafficRankDay>(
                  value: TrafficRankDay.today,
                  label: Text(L10n.t('今日')),
                ),
                ButtonSegment<TrafficRankDay>(
                  value: TrafficRankDay.yesterday,
                  label: Text(L10n.t('昨日')),
                ),
              ],
              selected: <TrafficRankDay>{_day},
              onSelectionChanged: (Set<TrafficRankDay> selection) {
                setState(() => _day = selection.first);
              },
              showSelectedIcon: false,
            ),
          ),
...
class _RankList extends StatelessWidget {
  ...
  final String title;
  final List<TrafficRankRow> rows;
  final Color color;

  @override
  Widget build(BuildContext context) {
    final ThemeData theme = Theme.of(context);
    final int maxBytes = rows.isEmpty ? 0 : rows.first.bytes;
      ...
      children: <Widget>[
        ...
        if (rows.isEmpty)
          ...
          for (int i = 0; i < rows.length; i++) ...<Widget>[
            ...
            _TrafficBarRow(
              label: rows[i].name,
              value: Format.bytes(rows[i].bytes).replaceAll(' ', ''),
              ratio: maxBytes <= 0 ? 0 : rows[i].bytes / maxBytes,
              barColor: color,
              tagColor: color,
            ),
          ],
                        ...
                        Row(
                          children: <Widget>[
                            ...
                            Expanded(
                              child: Text(
                                display,
                                maxLines: AppTheme.isWide(context) ? 1 : 3,
                                overflow: TextOverflow.ellipsis,
