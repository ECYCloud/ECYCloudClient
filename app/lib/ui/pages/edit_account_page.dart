...
class _EditAccountPageState extends State<EditAccountPage> {
  ...
  Future<void> _pollTelegram() async {
    ...
    try {
      final ({bool bound, String imValue, int telegramId}) result = await api
          .telegramBindCheck();
    ...
    if (enable == 1) {
      final bool? confirmed = await showDialog<bool>(
        ...
        builder: (BuildContext context) => AlertDialog(
          ...
          content: Text(L10n.t('开启后每次登录都需要输入验证器上的 6 位验证码。请确认已完成测试并通过验证。')),
          actions: <Widget>[
            FilledButton(
  ...
  Future<void> _resetGa() async {
    ...
    setState(() => _busy = true);
    try {
      final ({String message, String gaToken, String gaUrl}) result = await api
          .gaReset();
      if (!mounted) {
        return;
      }
  ...
  Future<void> _confirmTelegramUnbind(EditAccountOptions edit) async {
    ...
    setState(() => _busy = true);
    try {
      final ({String message, String bindToken}) result = await api
          .telegramReset();
      if (!mounted) {
        return;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            result.message.isEmpty ? L10n.t('已解绑') : result.message,
          ),
          behavior: SnackBarBehavior.floating,
        ),
      );
  ...
  List<MapEntry<String, String>> _visibleMailKeys(EditAccountOptions opts) {
    ...
    final bool showTrafficGroup =
        limitMode?.toString() != 'false' ||
                                    ...
                                    children: <Widget>[
                                      ...
                                      SizedBox(
                                        ...
                                        child: OutlinedButton(
                                          onPressed:
                                              _busy || _oldEmailCountdown > 0
                                              ? null
                                              : () => unawaited(
                                                  _run(
                                                    (PanelApiClient api) => api
                                                        .sendOldEmailVerify(),
                                                    onSuccess: () =>
                        ...
                        SectionCard(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              Text(
                                L10n.t(
                                  _options?.enablePasswordChangeResetSubscription ==
                                          true
                                      ? '修改密码后将重置订阅链接与节点密码，并需重新登录'
                                      : '修改密码后需重新登录',
                                ),
                                ...
                                Row(
                                  children: <Widget>[
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () => unawaited(
                                        _copy(L10n.t('otpauth 链接'), edit.gaUrl),
                                      ),
...
class _TelegramBoundLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasUsername = im.isNotEmpty && im != '未设置用户名' && im != '$id';

    final List<Widget> links = <Widget>[
      if (hasUsername) _TelegramLink(label: '@$im', url: 'https://t.me/$im'),
