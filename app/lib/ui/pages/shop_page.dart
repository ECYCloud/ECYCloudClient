...
class _ShopPageState extends State<ShopPage> with AutoRefreshMixin<ShopPage> {
  ...
  void didChangeDependencies() {
    ...
    if (!_started) {
      ...
      if (_catalog == null || !(_session?.fresh ?? false)) {
        unawaited(_load(silent: _catalog != null));
      }
    }
  }

  void _restoreSession() {
    final String? key = AppScope.of(context).auth.accountKey;
    final _ShopSession? session = _session;
    if (session == null) {
      return;
    }
    if (key == null || session.accountKey != key) {
      _session = null;
      return;
    }
    _catalog = session.catalog;
    _duration = session.duration;
    _cooldownUntil = session.cooldownUntil;
    _syncTicker(session.catalog);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(onRefresh: _load, child: _body(catalog));
  }
