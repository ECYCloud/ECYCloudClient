...
class NodeLabels {
  ...
  /// [nodeLabels]：官方客户端 API 的 proxy 名 => 显示名（proxy 名为 node-{id}）。
  /// 面板对本客户端按节点 ID 命名 proxy，这样管理员改节点名时内核看到的身份不变，
  /// 用户在策略组里选中的节点不会因改名而回落到首个成员。
  ...
  /// [tagOrName] 可能是内核里的 proxy 名（node-12）或已是显示名。
  ...
  /// 展示用的名字：先把 proxy 名解析为面板下发的节点名，再去掉自带的国旗 emoji。
  ...
  static String annotateRuntimeConfig(String raw) {
    if (_nodeNames.isEmpty || raw.isEmpty) {
      return raw;
    }
    final List<MapEntry<String, String>> labels = <MapEntry<String, String>>[
