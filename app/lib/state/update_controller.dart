...
class UpdateController extends ChangeNotifier {
  ...
  String get appStatus {
    ...
    return app.installer == null
        ? '已发布 ${app.latest}，但该版本未提供 ${AppUpdate.arch} 安装包'
  ...
  String? get kernelUpgradable =>
      _kernel?.outdated == true ? _kernel!.latest : null;

  bool get kernelUpgrading => _kernelUpgrading;

  /// 有未告知过的新版本。打开界面时据此弹一次窗，同一批版本不重复弹。
  bool get shouldAnnounce => _announceKey.isNotEmpty && _announceKey != _announcedKey;

  void markAnnounced() => _announcedKey = _announceKey;

  String get _announceKey {
    final String app = _app?.outdated == true ? 'app${_app!.latest}' : '';
    final String kernel = kernelUpgradable == null ? '' : 'kernel$kernelUpgradable';
    return '$app$kernel';
  }
  ...
  Future<void> checkKernel() async {
    try {
      final KernelUpdate update = await _connection.checkKernelUpdate();
      _kernel = update;
