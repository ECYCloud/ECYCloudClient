...
class AssembledProfile {
  ...
  int get remoteRuleSetCount => countRemoteRuleSets(config);

  static int countRemoteRuleSets(Map<String, dynamic> config) {
...
class ProfileAssembler {
  ...
  AssembledProfile assemble({
    ...
    final List<Object?> cleanedOutbounds = _withoutWebsocketEarlyData(
      outbounds,
    );
    final Map<String, dynamic> route = _route(
      remote,
      template,
      outbounds: cleanedOutbounds,
    );
  ...
  Map<String, dynamic> _route(
    ...
    route.addAll(template.routeOverrides);
    _ensureIcmpDirectRule(route, outbounds);
    return route;
  }

  // TUN 会吸入 ICMP（ping 等）。VMess/Trojan/selector 等代理出站不支持 ICMP，
  // 落到 route.final 时内核每秒打 WARN。壳层前置直连，不改面板其它分流规则。
  static void _ensureIcmpDirectRule(
    Map<String, dynamic> route,
    List<Object?> outbounds,
  ) {
    final String? directTag = _directOutboundTag(outbounds);
    if (directTag == null) {
      return;
    }

    final Object? rawRules = route['rules'];
    final List<Object?> rules = rawRules is List
        ? List<Object?>.from(rawRules)
        : <Object?>[];

    // 面板若已声明任意 ICMP 规则（含直连 / reject / WireGuard 等），壳层不覆盖
    final bool already = rules.any((Object? rule) {
      if (rule is! Map<String, dynamic>) {
        return false;
      }
      final Object? network = rule['network'];
      if (network == 'icmp') {
        return true;
      }
      return network is List && network.contains('icmp');
    });
    if (already) {
      route['rules'] = rules;
      return;
  ...
  static String? _directOutboundTag(List<Object?> outbounds) {
    for (final Object? outbound in outbounds) {
      if (outbound is Map<String, dynamic> &&
